...
@section('content')
<script src="{{Lottery::getAssets('js/red_envelopes_rain.min.js')}}"></script>
<div>
    <div class="main container" style="position:relative;margin:10px auto;">
        <div id="redEnvelopesRain" style="position:absolute;top:0;left:0;width: 100%;">
            <div id="vcc_alert_body_redEnvelopesRain" style="text-align:center;width: 100%;height:100%;min-height: 1000px;position: fixed;top: 0;left: 0;z-index: 990;background-color: rgba(0, 0, 0, 0.3);"></div>
            <div style="width: 100%;">
                <div vcc_alert_run="1" style="width: 500px;margin:auto;">
                    <div vcc_alert_content="1" vcc_prize_hint="true" style="position:fixed;top: 50%;left: 50%;transform: translate(-50%,-50%);z-index:991;width: 500px;display:none;padding:15px;text-align:center;font-size:24px;-webkit-border-radius: 8px;-moz-border-radius: 8px;border-radius: 8px;background:#fff;background-size: 100% 100%;">
                        <a>
                            <i vcc_alert_click="hide" vcc_prize_game="continuance" class="vcc_iconfont" style="position:absolute;top:2px;right:2px;cursor:default">&#xe63a;</i>
                        </a>
                        <p vcc_prize_hint="title" style="font-weight: 800;width: 90%;margin:auto;"></p>
                        <p vcc_prize_hint="code" style="font-weight: 500;color:#c00;"></p>
                        <p vcc_prize_hint="desc" style="font-size:16px;"></p>
                ...
                <div style="width: 1024px; height: 1000px;margin:0 auto;">
                    <div id="canvas_prize1" style="width: 1024px; height: 1000px;position:fixed;top:0;z-index:990;">
                        <i class="vcc_iconfont" vcc_canvas_prize1_click_hide="true" style="position:absolute;top:6px;right:6px;font-size:63px;color:#fff;cursor:default">&#xe63a;</i>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    var callback = {
        ...
        prizeHint:function(data)
        {
            ...
            obj.find('[vcc_prize_hint="code"]').html('code：'+data['code']);
        ...
        {
            var obj = $('[vcc_prize_hint="true"]');
            obj.find('[vcc_prize_hint="title"]').html('没有中奖资格或中奖次数以用完');
            obj.find('[vcc_prize_hint="code"]').hide();
            obj.find('[vcc_prize_hint="desc"]').html('每<span style="color:#f00;font-weight:800;">1</span>人<span style="color:#f00;font-weight:800;">1</span>天<span style="color:#f00;font-weight:800;">10</span>次机会。');
            obj.find('[vcc_prize_hint="click1"]').hide();
            obj.find('[vcc_prize_hint="click2"]').hide();
            $('[vcc_alert_click="hide"]').click(function(){
        ...
        {
            $('[vcc_prize_game="continuance"]').click(function(){
                if(i>0){
                    game.paused = false;
                }else{
                    game.paused = true;
    ...
    var data_img = {'jing_ling1':"{{Lottery::getAssets('img/honbao.png')}}"};
